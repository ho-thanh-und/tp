...
We also gratefully acknowledge the creators of the following resources, libraries, and tools that were instrumental in bringing EduVault to life:
...
* inherits from both `AddressBookStorage`, `UserPrefStorage`, and `ScheduleBoardStorage` which means it can be treated as either one (if only the functionality of only one is needed).
